...
Company.destroy_all
Post.destroy_all
stlaurent=City.find_or_create_by(name: "Saint-Laurent-du-Maroni")
mana=City.find_or_create_by(name: "Mana")

#j Fetch and parse HTML documen``t
`rm index.html; curl https://www.saintlaurentdumaroni.fr/ --output index.html;`
doc = Nokogiri::HTML(File.read("./index.html"))
@doc = Nokogiri::HTML(File.read("./index.html"))
 p "cool"

# Search for nodes by css
 p doc.css('.eau .cel1 a').length
 p "items"
doc.css('.eau .cel1 a').each do |link|
  #puts link.content
  p "www.saintlaurentdumaroni.fr#{URI.parse(escape(link.attributes['href'].value))}"
 p "ok"
 `rm myfile.html; curl "https://www.saintlaurentdumaroni.fr#{escape(link.attributes['href'].value)}" --output myfile.html;`
  x="./myfile.html"
  doc2 = Nokogiri::HTML(File.read(x))
  k=Post.create do |h|
    h.image=doc2.css("img")[8].attributes["src"].value
    h.title=doc2.css("h1")[0].inner_html
    h.content=doc2.css(".texte, .pj").map{|b|b.inner_html}.join("")
    h.date=doc2.css(".titre .date").inner_html
    h.city_id=stlaurent.id
  end
  p k
  end
